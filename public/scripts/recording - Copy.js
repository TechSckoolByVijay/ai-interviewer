    ...
    return new Promise((resolve, reject) => {
        try {
            if (!mediaStreams.camera) {
                reject(new Error('Camera stream not initialized'));
                return;
            }

            // Initialize or reuse video element
            if (!videoElement) {
            ...
            console.error('Error in setupCameraDisplay:', error);
            reject(error);
        }
    });
}
...
function startDrawingLoop() {
    ...
    function drawFrame() {
        if (!videoElement || !ctx || !isInterviewActive) return;
...
async function initializeInterview() {
    try {
        ...
        if (!mediaStreams.camera) {
            ...
            await setupCameraDisplay(); // Wait for camera setup
        }

        // Then initialize other streams
