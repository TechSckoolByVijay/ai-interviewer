...
async def get_section_questions(
    ...
    answered_questions = db.query(InterviewQuestion).filter(
        ...
        InterviewQuestion.status == QuestionStatus.ANSWERED
    ).all()

    # Get all sections that have been completed
    completed_sections = set(q.section for q in answered_questions)

    # Get the next available section
    available_sections = list(SECTIONS.keys())
    
    if not current_section:
        # For first request, return first section
    ...
    if next_section in completed_sections:
        return {
            "has_more": False,
            "current_section": None,
            "questions": []
    ...
    # Get questions for the next section
    questions = SECTIONS.get(next_section, [])
    if not questions:
        raise HTTPException(status_code=404, detail=f"No questions found for section: {next_section}")

    # Create/Get questions in database
    db_questions = []
    for question_text in questions:
        existing_question = db.query(InterviewQuestion).filter(
            InterviewQuestion.interview_id == interview_id,
            InterviewQuestion.section == next_section,
        ...
        if not existing_question:
            ...
            db_questions.append(existing_question)

    db.commit()

    # Check if this is the last section
    is_last_section = next_section == available_sections[-1]

    return {
        "has_more": not is_last_section,
